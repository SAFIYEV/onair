...
const GameData = {
    ...
    damageMultiplier: 1,
    missions: {
        1: { killed: 0, completed: false },      // Убить 10 зомби
        2: { score: 0, completed: false },       // Набрать 1000 очков
        3: { time: 0, completed: false },        // Выжить 5 минут
        4: { completed: false },                 // Подписка на YouTube
        5: { completed: false },                 // Подписка на SADLAHAY_CODE
        6: { completed: false },                 // Подписка на Сафиев Марат
        7: { completed: false },                 // Подписка на CryptoWorld
        8: { completed: false },                 // Присоединиться к ONECOIN комьюнити
        9: { killed: 0, completed: false },      // Убить 50 зомби
        10: { killed: 0, completed: false },     // Убить 100 зомби
        11: { score: 0, completed: false },      // Набрать 5000 очков
        12: { time: 0, completed: false },       // Выжить 10 минут
        13: { streak: 0, completed: false },     // Убить 10 зомби без перезарядки
        14: { quickKills: 0, completed: false }, // Убить 5 зомби за 10 секунд
        15: { distance: 0, completed: false },   // Пройти 1000 метров
        16: { noHitKills: 0, completed: false }, // Убить 20 зомби без получения урона
        17: { score: 0, completed: false },      // Набрать 10000 очков
        18: { time: 0, completed: false },       // Выжить 15 минут
        19: { killed: 0, completed: false },     // Убить 200 зомби
        20: { level: 1, completed: false }       // Достичь 20 уровня
    },
...
class ZombieShooter {
    constructor() {
        ...
        // Добавляем новые игровые переменные
        this.streak = 0;           // Убийства без перезарядки
        this.quickKills = 0;      // Быстрые убийства
        this.lastKillTime = 0;    // Время последнего убийства
        this.distance = 0;        // Пройденная дистанция
        this.noHitStreak = 0;     // Убийства без получения урона
        this.level = 1;           // Текущий уровень игрока
    ...
    reload() {
        ...
        this.streak = 0; // Сброс серии убийств без перезарядки
            ...
            for (let j = this.zombies.length - 1; j >= 0; j--) {
                ...
                if (bullet.position.distanceTo(zombie.position) < 1) {
                    ...
                    if (zombie.health <= 0) {
                        ...
                        // Обновление прогресса всех заданий на убийство
                        this.updateKillMissions();
                        
                        // Обновление прогресса заданий на очки
                        this.updateScoreMissions();
                        
                        // Обновление быстрых убийств
                        const currentTime = performance.now();
                        if (currentTime - this.lastKillTime < 10000) { // 10 секунд
                            this.quickKills++;
                            if (this.quickKills >= 5 && !GameData.missions[14].completed) {
    ...
    updateKillMissions() {
        ...
        if (!GameData.missions[1].completed) {
            ...
            if (this.zombiesKilled >= 10) this.completeMission(1);
        }
        
        // Убить 50 зомби
        if (!GameData.missions[9].completed) {
            GameData.missions[9].killed = this.zombiesKilled;
            if (this.zombiesKilled >= 50) this.completeMission(9);
        }
        
        // Убить 100 зомби
        if (!GameData.missions[10].completed) {
            GameData.missions[10].killed = this.zombiesKilled;
            if (this.zombiesKilled >= 100) this.completeMission(10);
        }
        
        // Убить 200 зомби
        if (!GameData.missions[19].completed) {
            GameData.missions[19].killed = this.zombiesKilled;
            if (this.zombiesKilled >= 200) this.completeMission(19);
        }
        
        // Убийства без перезарядки
        this.streak++;
        if (this.streak >= 10 && !GameData.missions[13].completed) {
            this.completeMission(13);
        }
        
        // Убийства без получения урона
        if (this.health === GameData.maxHealth) {
            this.noHitStreak++;
            if (this.noHitStreak >= 20 && !GameData.missions[16].completed) {
    ...
    updateScoreMissions() {
        ...
        // 5000 очков
        if (!GameData.missions[11].completed && this.score >= 5000) {
            this.completeMission(11);
        }
        
        // 10000 очков
        if (!GameData.missions[17].completed && this.score >= 10000) {
            this.completeMission(17);
        }
        
        // Обновление уровня
        this.level = Math.floor(this.score / 1000) + 1;
        if (this.level >= 20 && !GameData.missions[20].completed) {
    ...
    updateMovement() {
        ...
        if (this.moveForward || this.moveBackward || this.moveLeft || this.moveRight) {
            ...
            if (!GameData.missions[15].completed && this.distance >= 1000) {
    ...
    animate() {
        ...
        // Обновление времени для всех заданий на выживание
        this.updateTimeMissions(deltaTime);
    ...
    updateTimeMissions(deltaTime) {
        this.gameTime += deltaTime;
        
        // 5 минут
        if (!GameData.missions[3].completed && this.gameTime >= 300) {
            this.completeMission(3);
        }
        
        // 10 минут
        if (!GameData.missions[12].completed && this.gameTime >= 600) {
            this.completeMission(12);
        }
        
        // 15 минут
        if (!GameData.missions[18].completed && this.gameTime >= 900) {
...
window.addEventListener('load', () => {
    ...
    document.querySelector('.share-btn').addEventListener('click', () => {
        if (navigator.share) {
            navigator.share({
                title: 'Шутер',
                text: 'Попробуй крутой шутер! Я уже набрал ' + GameData.totalScore + ' очков!',
                url: window.location.href
            });
        } else {
            // Запасной вариант для браузеров без API шаринга
            const shareText = `Шутер - Попробуй крутой шутер! Я уже набрал ${GameData.totalScore} очков!\n${window.location.href}`;
